...
One command. It checks your prerequisites, pulls the images, and writes a
`.env` with secrets generated by `openssl rand -base64 48`:
...
**Change that password before anyone else can reach the install** — it is
seeded on first start and documented right here.

First start takes a few minutes: it runs every database migration and seeds the
default roles, users and vulnerability categories.

Docker is the only requirement. The images cover `linux/amd64` and
`linux/arm64`, so the same command works on an Apple Silicon Mac, an Intel
laptop and a cloud server.

<details>
<summary>Options, and what to do instead of piping to a shell</summary>
...
Piping anything to `bash` means running code you have not read, and it is
reasonable not to want to — particularly for a security tool. Read it first:
...
The script starts nothing and writes only inside its install directory. Run it
twice and it refuses to touch an existing `.env`, because regenerating
`DATABASE_PASSWORD` would lock a working install out of its own database.

Or skip it and do the same three things by hand — copy `.env.example` to
`.env`, fill in the three secrets, `docker compose up -d`. See
[DOCKER_QUICK_START.md](DOCKER_QUICK_START.md), which also covers upgrading and
pinning versions.
